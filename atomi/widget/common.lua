...
function common.list_update(w, font, dpi_, buttons, label, data, objects)
    ...
    for i, o in ipairs(objects) do
        ...
        if cache then
            ...
            ib = proton.widget.imagebox()
            tb = proton.widget.textbox(font)
            bgb = proton.container.background()
            tbm = proton.container.margin(tb, dpi(4, dpi_), dpi(4, dpi_))
            ibm = proton.container.margin(ib, dpi(4, dpi_))
            l = proton.layout.fixed.horizontal()
