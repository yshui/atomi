...
-- @module atomi.hotkeys_popup.widget
---------------------------------------------------------------------------
...
local compute_textbox_width = require("atomi.menubar").utils.compute_textbox_width
...
local widget = {
    ...
    width = dpi(1200),
    height = dpi(800),
    border_width = ugly.border_width or dpi(2),
    modifiers_color = ugly.bg_minimize or "#555555",
    group_margin = dpi(6),
    group_rules = {},
    additional_hotkeys = {},
    labels = {
...
local colors = ugly.x.get_current_theme()
local group_list = {}
...
local function import_awful_keys()
    ...
    cached_awful_keys = {}
    for _, data in pairs(atomi.key.hotkeys) do
...
local function create_wibox(s, available_groups)
    ...
    for _, group in ipairs(available_groups) do
        ...
        -- +1 for group label:
        local items_height = atomi.util.linecount(joined_descriptions) * line_height + group_label_height
        ...
        if not current_column then
            current_column = {layout=proton.layout.fixed.vertical()}
        end
        ...
        local function insert_keys(_keys, _add_new_column)
            ...
            current_column.height_px = (current_column.height_px or 0) +
                atomi.util.linecount(joined_labels)*line_height + group_label_height
    ...
    local pages = {}
    local columns = proton.layout.fixed.horizontal()
    for _, item in ipairs(column_layouts) do
        ...
        local column_margin = proton.container.margin()
    ...
    mywibox:buttons(atomi.util.table.join(
            atomi.button({ }, 1, function () myproton.visible=false end),
            atomi.button({ }, 3, function () myproton.visible=false end)
    ))

    local widget_obj = {}
    widget_obj.current_page = 1
    widget_obj.proton = mywibox
    function widget_obj:page_next()
        if self.current_page == #pages then return end
        self.current_page = self.current_page + 1
        self.proton:set_widget(pages[self.current_page])
    end
...
function widget.show_help(c, s)
    ...
    s = s or (c and c.screen or atomi.screen.focused())
    ...
    for group, _ in pairs(group_list) do
        ...
        for group_name, data in pairs(widget.group_rules) do
            if group_name==group and (
                ...
                if not c or not atomi.rules.matches(c, {
                    rule=data.rule,
...
-- see `atomi.hotkeys_popup.key.vim` as an example.
