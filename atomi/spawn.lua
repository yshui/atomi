...
--
-- See `atomi.rules.execute` for more details about the format of `sn_rules`.
--
...
-- This calls `cmd` with `$SHELL -c` (via `atomi.util.shell`).
...
function spawn.read_lines(input_stream, line_callback, done_callback, close)
    ...
    finish_read = function(obj, res)
        ...
        if type(length) ~= "number" then
            -- Error
            print("Error in atomi.spawn.read_lines:", tostring(length))
            done()
