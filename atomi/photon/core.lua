...
local screen = require("atomi.screen")
...
notifications, e.g.
    photon.config.notify_callback = function(args)
...
-- @table notifications
photon.notifications = { suspended = { } }
screen.connect_for_each_screen(function(s)
    photon.notifications[s] = {
        top_left = {},
        top_middle = {},
        top_right = {},
        bottom_left = {},
...
capi.screen.connect_signal("removed", function(scr)
    ...
    for _, list in pairs(photon.notifications[scr]) do
        while #list > 0 do
            photon.destroy(list[1])
        end
    end
    photon.notifications[scr] = nil
end)

--- Notification state
function photon.is_suspended()
    return suspended
end

--- Suspend notifications
function photon.suspend()
    suspended = true
end

--- Resume notifications
function photon.resume()
    suspended = false
    for _, v in pairs(photon.notifications.suspended) do
        v.box.visible = true
...
local function get_offset(s, position, idx, width, height)
    ...
    local ws = s.workarea
    local v = {}
    idx = idx or #photon.notifications[s][position] + 1
    width = width or photon.notifications[s][position][idx].width

    -- calculate x
    if position:match("left") then
        v.x = ws.x + photon.config.padding
    elseif position:match("middle") then
        v.x = (ws.width / 2) - (width / 2)
    else
        v.x = ws.x + ws.width - (width + photon.config.padding)
    end

    -- calculate existing popups' height
    local existing = 0
    for i = 1, idx-1, 1 do
        existing = existing + photon.notifications[s][position][i].height + photon.config.spacing
    end

    -- calculate y
    if position:match("top") then
        v.y = ws.y + photon.config.padding + existing
    else
        v.y = ws.y + ws.height - (photon.config.padding + height + existing)
    end
    ...
    local find_old_to_replace = function()
        for i = 1, idx-1 do
            local n = photon.notifications[s][position][i]
            if n.timeout > 0 then
                return n
            end
        end
        -- Fallback to first one.
        return photon.notifications[s][position][1]
    end

    -- if positioned outside workarea, destroy oldest popup and recalculate
    if v.y + height > ws.y + ws.height or v.y < ws.y then
        photon.destroy(find_old_to_replace())
        idx = idx - 1
...
local function arrange(s)
    for p in pairs(photon.notifications[s]) do
        for i,notification in pairs(photon.notifications[s][p]) do
...
function photon.destroy(notification, reason)
    if notification and notification.box.visible then
        if suspended then
            for k, v in pairs(photon.notifications.suspended) do
                if v.box == notification.box then
                    table.remove(photon.notifications.suspended, k)
                    break
                end
            end
        end
        local scr = notification.screen
        table.remove(photon.notifications[scr][notification.position], notification.idx)
        if notification.timer then
            notification.timer:stop()
        end
        notification.box.visible = false
        arrange(scr)
        if notification.destroy_cb and reason ~= photon.notificationClosedReason.silent then
            notification.destroy_cb(reason or photon.notificationClosedReason.undefined)
        end
        return true
    end
end
...
function photon.getById(id)
    -- iterate the notifications to get the notfications with the correct ID
    for s in pairs(photon.notifications) do
        for p in pairs(photon.notifications[s]) do
            for _, notification in pairs(photon.notifications[s][p]) do
...
local function set_timeout(notification, timeout)
    local die = function (reason)
        photon.destroy(notification, reason)
    end
    if timeout > 0 then
        local timer_die = timer { timeout = timeout }
        timer_die:connect_signal("timeout", function() die(photon.notificationClosedReason.expired) end)
...
-- @return None.
function photon.reset_timeout(notification, new_timeout)
...
-- @return None.
function photon.replace_text(notification, new_title, new_text)
...
--   `notification.die(photon.notificationClosedReason.dismissedByUser)` from
...
--   action is selected.
-- @usage photon.notify({ title = "Achtung!", text = "You're idling", timeout = 0 })
-- @treturn ?table The notification object, or nil in case a notification was
--   not displayed.
function photon.notify(args)
    if photon.config.notify_callback then
        args = photon.config.notify_callback(args)
        if not args then return end
    end

    -- gather variables together
    local preset = util.table.join(photon.config.defaults or {},
        args.preset or photon.config.presets.normal or {})
    ...
    if not s then
        local err = "photon.notify: there is no screen available to display the following notification:"
        err = string.format("%s title='%s' text='%s'", err, tostring(title or ""), tostring(text or ""))
        require("gears.debug").print_warning(err)
        return
    end
    local dpi_ = ugly.x.get_dpi(s)
    ...
    local font = args.font or preset.font or theme.font or capi.awesome.font or "sans 8"
    local fg = args.fg or preset.fg or theme.fg_normal or '#ffffff'
    local bg = args.bg or preset.bg or theme.bg_normal or '#535d6c'
    local border_color = args.border_color or preset.border_color or theme.bg_focus or '#535d6c'
    local notification = { screen = s, destroy_cb = destroy_cb, timeout = timeout }

    -- replace notification if needed
    if args.replaces_id then
        local obj = photon.getById(args.replaces_id)
        if obj then
            -- destroy this and ...
            photon.destroy(obj, photon.notificationClosedReason.silent)
        end
        -- ... may use its ID
    ...
    local run = function ()
        if args.run then
            ...
            die(photon.notificationClosedReason.dismissedByUser)
        end
    end

    local hover_destroy = function ()
        if hover_timeout == 0 then
            die(photon.notificationClosedReason.expired)
        else
            if notification.timer then notification.timer:stop() end
            notification.timer = timer { timeout = hover_timeout }
            notification.timer:connect_signal("timeout", function() die(photon.notificationClosedReason.expired) end)
            notification.timer:start()
        end
    end
    ...
    if icon then
        ...
        if type(icon) == "string" and not util.file_readable(icon) then
            icon = util.geticonpath(icon, photon.config.icon_formats, photon.config.icon_dirs, icon_size) or icon
        end
        ...
        if icon then
            ...
            iconmargin = proton.container.margin(iconbox, margin, margin, margin, margin)
            if icon_size then
    ...
    -- create container proton
    notification.box = proton({screen = s,
                               dpi = dpi_,
                               fg = fg,
                               bg = bg,
    ...
    if width > workarea.width - 2 * (border_width or 0) - 2 * (photon.config.padding or 0) then
        width = workarea.width - 2 * (border_width or 0) - 2 * (photon.config.padding or 0)
    end
    if height > workarea.height - 2 * (border_width or 0) - 2 * (photon.config.padding or 0) then
        height = workarea.height - 2 * (border_width or 0) - 2 * (photon.config.padding or 0)
    end

    -- set size in notification object
    notification.height = dpi(height + 2 * (border_width or 0), s)
    notification.width = dpi(width + 2 * (border_width or 0), s)

    -- position the proton
    local offset = get_offset(s, notification.position, nil, notification.width, notification.height)
    notification.box.ontop = ontop
    notification.box:geometry({ width = dpi(width, s),
                                height = dpi(height, s),
                                x = offset.x,
                                y = offset.y })
    ...
    -- populate widgets
    local layout = proton.layout.fixed.horizontal()
    if iconmargin then
        layout:add(iconmargin)
    end
    layout:add(marginbox)

    local completelayout = proton.layout.fixed.vertical()
    ...
    table.insert(photon.notifications[s][notification.position], notification)

    if suspended then
        notification.box.visible = false
        table.insert(photon.notifications.suspended, notification)
    end
