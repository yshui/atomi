...
dbus.config.mapping = {
    {{urgency = urgency.low}, photon.config.presets.low},
    {{urgency = urgency.normal}, photon.config.presets.normal},
    {{urgency = urgency.critical}, photon.config.presets.critical}
}
...
capi.dbus.connect_signal("org.freedesktop.Notifications", function (data, appname, replaces_id, icon, title, text, actions, hints, expire)
    ...
    if data.member == "Notify" then
        ...
        local preset = args.preset or photon.config.defaults
        local notification
        if actions then
            args.actions = {}
            ...
            for i = 1,#actions,2 do
                ...
                if action_id == "default" then
                    args.run = function()
                        ...
                        photon.destroy(notification, photon.notificationClosedReason.dismissedByUser)
                    end
        ...
        if not preset.callback or (type(preset.callback) == "function" and
            ...
            notification = photon.notify(args)
            return "u", notification.id
        end
        return "u", "0"
    elseif data.member == "CloseNotification" then
        local obj = photon.getById(appname)
        if obj then
           photon.destroy(obj, photon.notificationClosedReason.dismissedByCommand)
        end
    elseif data.member == "GetServerInfo" or data.member == "GetServerInformation" then
        -- name of notification app, name of vender, version, specification version
        return "s", "photon", "s", "awesome", "s", capi.awesome.version, "s", "1.0"
