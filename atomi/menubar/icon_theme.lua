...
local index_theme = require("atomi.menubar.index_theme")
...
local get_pragmatic_base_directories = function()
    ...
    if autil.dir_readable(dir) then
    ...
    if need_usr_share_pixmaps and autil.dir_readable(dir) then
        table.insert(dirs, dir)
    end
...
local get_default_icon_theme_name = function()
    ...
    for _, dir in ipairs(get_pragmatic_base_directories()) do
        for _, icon_theme_name in ipairs(icon_theme_names) do
            ...
            if autil.file_readable(filename) then
...
icon_theme.new = function(icon_theme_name, base_directories)
    icon_theme_name = icon_theme_name or ugly.icon_theme or get_default_icon_theme_name()
