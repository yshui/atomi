...
--   a description for atomi.proton.pattern or a cairo pattern.
...
--   atomi.proton.pattern. The default is black.
...
function wallpaper.mt:__call(s)
    local wp = ugly.wallpaper
    if type(wp) == "function" then
        wp = wp(s)
    end
    if ugly.wallpaper_style == "tiled" or
        ugly.wallpaper_style == "center" or
        ugly.wallpaper_style == "fit" or
        ugly.wallpaper_style == "maximized" then
        wallpaper[ugly.wallpaper_style](wp, s)
    end
end
